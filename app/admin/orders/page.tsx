            ...
            {orders.length === 0 ? (
              ...
              orders.map((order: any) => (
                <tr key={order.id} className="hover:bg-gray-50">
                  ...
                  <td className="p-4">
                    <div className="space-y-1 text-xs">
                      <div className="flex items-center gap-2 text-gray-700">
                        <User size={12}/> {order.users?.username || 'Unknown'}
                      </div>
                      <div className="flex items-center gap-2 text-orange-600">
                        <Store size={12}/> {order.shops?.name || 'Unknown Shop'}
                      </div>
                      {order.driver_id && (
                  ...
                  {/* FIX IS HERE: Added .toFixed(2) */}
                  <td className="p-4 font-bold">RM {Number(order.total_amount).toFixed(2)}</td>
                  <td className="p-4">
