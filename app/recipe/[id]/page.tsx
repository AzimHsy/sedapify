...
import { ArrowLeft, ChefHat, Sparkles, ShoppingBasket } from "lucide-react";
import RecipeInteraction from "@/components/RecipeInteraction";
import RecipeCard from "@/components/RecipeCard";
import DietaryGuard from "@/components/DietaryGuard";
// REMOVED: CommentSection import

// ... ErrorDisplay ... (Keep this at bottom or top)
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-[#FDF8F0] p-4 text-center">
      <div className="bg-white p-8 rounded-3xl shadow-xl max-w-md w-full border border-orange-100">
        ...
        <Link href="/" className="inline-block w-full bg-orange-500 text-white px-6 py-3 rounded-xl font-bold hover:bg-orange-600 transition">Go Back Home</Link>
      </div>
    </div>
  );
}
...
export default async function RecipePage({
  ...
  // ... (Keep existing ID validation) ...
  const isValidUUID = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(id);
  if (!isValidUUID) return <ErrorDisplay title="Invalid Recipe ID" message="The ID provided is not valid." />;

  const supabase = await createClient();

  // 1. Fetch User (Keep existing logic)
  const { data: { user } } = await supabase.auth.getUser();
  
  let userDietaryPref = 'Standard';
  if (user) {
      const { data: profile } = await supabase.from('users').select('dietary_pref').eq('id', user.id).single();
  ...
  // ... (Keep existing Like/Save/Suggestions logic) ...
  let isLiked = false;
  let isSaved = false;
  if (user) {
          ...
          <div className="max-w-7xl mx-auto px-4 md:px-10 -mt-8 relative z-20 mb-16">
             <div className="bg-white rounded-3xl shadow-xl overflow-hidden border border-gray-100 p-6 md:p-10">
                <div className="grid lg:grid-cols-3 gap-10">
                    ...
                    {/* LEFT: INGREDIENTS (Keep unchanged) */}
                    ...
                    <div className="lg:col-span-2">
                        <div className="flex items-center justify-between mb-6">
                            ...
                            <RecipeInteraction 
                                recipe={recipe} // <--- Passing full recipe object now
                                currentUser={user} 
                ...
                {/* REMOVED: CommentSection (It's now inside the modal) */}
